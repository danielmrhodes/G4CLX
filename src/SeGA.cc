...
void SeGA::PlaceDetector(G4LogicalVolume* expHall_log, G4int detNum, G4double Zoffset, G4bool check) { 
  ...
  for(G4int i=0;i<phisegs;i++) {
	...
	segDL_log->SetVisAttributes(Vis3);
...
void SeGA::Placement(G4LogicalVolume* expHall_log, G4double ZOff, G4bool check) {

  for(G4int i=0;i<16;i++)
    PlaceDetector(expHall_log,i,ZOff,check); 
